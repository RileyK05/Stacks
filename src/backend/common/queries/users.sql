-- name: create
INSERT INTO users (name, email, password_hash, password_changed_at)
VALUES (%(name)s, %(email)s, %(password_hash)s, now())
...
SELECT user_id, name, email, tier, email_verified_at, password_hash,
       password_changed_at, delete_requested_at, created_at
FROM users
